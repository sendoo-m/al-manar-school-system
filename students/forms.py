...
def validate_unique_optional_field(model, field_name, value, instance=None,
                                   message='هذه القيمة مستخدمة من قبل'):
...
# ============================================================
# Widgets المشتركة — تُعاد الاستخدام في كلا النموذجين
# ============================================================
...
class StudentBaseFormMixin:
    ...
    def setup_grade_queryset(self):
        self.fields['grade_level'].queryset = GradeLevel.objects.filter(
            is_active=True
        ).select_related('education_level').order_by(
            'education_level__order', 'order', 'name'
        )
    ...
    def clean_national_number(self):
        ...
        if national_number:
            ...
            return validate_unique_optional_field(
                Student, 'national_number', national_number,
    ...
    def clean_phone_number(self):
        return clean_optional_digits(
            self.cleaned_data.get('phone_number'), 'رقم الهاتف'
        )

    def clean_parent_phone(self):
        return clean_optional_digits(
            self.cleaned_data.get('parent_phone'), 'هاتف ولي الأمر'
        )
    ...
    def clean(self):
        ...
        student_type    = cleaned_data.get('student_type', 'REGULAR')
        national_number = (cleaned_data.get('national_number') or '').strip()
        passport_number = (cleaned_data.get('passport_number') or '').strip()

        if student_type == 'REGULAR' and not national_number:
            self.add_error('national_number', 'الرقم القومي مطلوب للطلاب المصريين')

        elif student_type == 'EXPATRIATE' and not passport_number:
            self.add_error('passport_number', 'رقم جواز السفر مطلوب للطلاب الوافدين')

        is_integration_student = cleaned_data.get('is_integration_student')
        disability_type        = (cleaned_data.get('disability_type') or '').strip()

        if disability_type and not is_integration_student:
            cleaned_data['is_integration_student'] = True
        elif not disability_type and is_integration_student is False:
            cleaned_data['is_integration_student'] = False

        is_staff_child    = cleaned_data.get('is_staff_child')
        staff_parent_name = (cleaned_data.get('staff_parent_name') or '').strip()
        staff_parent_job  = (cleaned_data.get('staff_parent_job') or '').strip()

        if (staff_parent_name or staff_parent_job) and not is_staff_child:
            cleaned_data['is_staff_child'] = True
        elif not staff_parent_name and not staff_parent_job and is_staff_child is False:
            cleaned_data['is_staff_child'] = False

        return cleaned_data


def _make_grade_level_field():
...
class StudentForm(StudentBaseFormMixin, forms.ModelForm):
    ...
    grade_level   = _make_grade_level_field()
    academic_year = _make_academic_year_field()

    class Meta:
        model = Student
        fields = [
            'name', 'student_type', 'national_number', 'passport_number',
            'nationality', 'religion', 'phone_number', 'address',
            'academic_year', 'grade_level', 'enrollment_status',
            'transferred_from_school', 'transferred_to_school',
            'is_integration_student', 'disability_type',
            'exempt_from_arabic', 'exempt_from_english', 'exempt_from_french',
            'other_subject_exemptions',
            'parent_name', 'parent_phone', 'parent_email', 'father_job',
            'educational_guardian', 'educational_guardian_name',
            'educational_guardian_phone',
            'is_staff_child', 'staff_parent_name', 'staff_parent_job',
        ]
...
class StudentEditForm(StudentBaseFormMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            'name', 'student_type', 'national_number', 'passport_number',
            'nationality', 'religion', 'gender', 'age', 'date_of_birth',
