...
class StudentExportService:
    """خدمة تصدير بيانات الطلاب مع دعم المرحلة التعليمية والصف الدراسي"""
    ...
    @classmethod
    def safe_attr(cls, obj, attr_name, default=''):
        return getattr(obj, attr_name, default) if obj else default

    @classmethod
    def safe_display(cls, obj, method_name, fallback_attr=''):
        if obj and hasattr(obj, method_name):
            try:
                ...
                pass
        if fallback_attr:
            return getattr(obj, fallback_attr, '') if obj else ''
        return ''
    ...
    def get_student_row(cls, student):
        ...
        return [
            student.name or '',
            cls.safe_display(student, 'get_student_type_display', 'student_type'),
            student.national_number or '',
            getattr(student, 'passport_number', '') or '',
            getattr(student, 'nationality', '') or '',
            cls.safe_display(student, 'get_religion_display', 'religion'),
            student.age or '',
            cls.safe_display(student, 'get_gender_display', 'gender'),
            ...
            cls.safe_display(student, 'get_enrollment_status_display', 'enrollment_status'),
            ...
            cls.safe_display(student, 'get_educational_guardian_display', 'educational_guardian'),
            getattr(student, 'educational_guardian_name', '') or '',
            getattr(student, 'educational_guardian_phone', '') or '',

            float(getattr(student, 'total_fees', 0) or 0),
            float(getattr(student, 'total_payments', 0) or 0),
            float(getattr(student, 'total_owed', 0) or 0),
            student.get_financial_status() if hasattr(student, 'get_financial_status') else '',

            cls.format_datetime(getattr(student, 'created_at', None)),
            cls.format_datetime(getattr(student, 'updated_at', None)),
    ...
    def get_student_dict(cls, student):
        ...
        return {
            ...
            'student_type_display': cls.safe_display(student, 'get_student_type_display', 'student_type'),
            ...
            'religion_display': cls.safe_display(student, 'get_religion_display', 'religion'),
            'age': student.age,
            'gender': student.gender or '',
            'gender_display': cls.safe_display(student, 'get_gender_display', 'gender'),
            'date_of_birth': student.date_of_birth.isoformat() if getattr(student, 'date_of_birth', None) else '',
            'phone_number': student.phone_number or '',
            'address': student.address or '',

            'academic_year': academic_year.name if academic_year else '',
            ...
            'enrollment_status_display': cls.safe_display(student, 'get_enrollment_status_display', 'enrollment_status'),
            ...
            'educational_guardian_display': cls.safe_display(student, 'get_educational_guardian_display', 'educational_guardian'),
            ...
            'total_fees': float(getattr(student, 'total_fees', 0) or 0),
            'total_payments': float(getattr(student, 'total_payments', 0) or 0),
            'total_owed': float(getattr(student, 'total_owed', 0) or 0),
            'financial_status': student.get_financial_status() if hasattr(student, 'get_financial_status') else '',

            'created_at': getattr(student, 'created_at', None).isoformat() if getattr(student, 'created_at', None) else '',
            'updated_at': getattr(student, 'updated_at', None).isoformat() if getattr(student, 'updated_at', None) else '',
    ...
    def export_csv(cls, queryset):
        ...
        response['Content-Disposition'] = (
            f'attachment; filename="students_export_{datetime.now().strftime("%Y%m%d_%H%M")}.csv"'
        )
        response.write('\ufeff')
    ...
    def export_excel(cls, queryset):
        ...
        response['Content-Disposition'] = (
            f'attachment; filename="students_export_{datetime.now().strftime("%Y%m%d_%H%M")}.xlsx"'
        )
    ...
    def export_json(cls, queryset):
        ...
        response['Content-Disposition'] = (
            f'attachment; filename="students_export_{datetime.now().strftime("%Y%m%d_%H%M")}.json"'
        )
