...
class StudentImportPreviewService:
    """خدمة معاينة واستيراد الطلاب مع دعم المرحلة التعليمية والصف الدراسي"""
    ...
    HEADER_ALIASES = {
        ...
        'education_level': ['المرحلة التعليمية', 'المرحلة', 'المرحلة الدراسية'],
    ...
    def preview_file(self, file_obj):
        ...
        for index, raw_row in enumerate(rows, start=2):
            ...
            education_level_name = normalized.get('education_level', '').strip()
            grade_name = normalized.get('grade_level', '').strip()

            education_level = self.find_education_level(education_level_name)
            grade = self.find_grade(
                grade_name=grade_name,
                education_level_name=education_level_name
            )

            if education_level_name and not education_level:
                row_warnings.append(f'لم يتم العثور على المرحلة التعليمية: {education_level_name}')

            if grade_name:
                if not grade:
                    if education_level_name:
                        row_warnings.append(
                            f'لم يتم العثور على الصف الدراسي: {grade_name} داخل المرحلة: {education_level_name}'
                        )
                    else:
                        row_warnings.append(f'لم يتم العثور على الصف الدراسي: {grade_name}')
                else:
                    normalized['grade_level'] = grade.name
                    normalized['grade_level_name'] = grade.name
                    normalized['education_level_name'] = (
                        grade.education_level.name if grade.education_level else ''
                    )
            else:
                normalized['grade_level_name'] = ''
                normalized['education_level_name'] = education_level_name or ''

            academic_year_name = normalized.get('academic_year', '').strip()
            academic_year = self.find_academic_year(academic_year_name)

            if academic_year_name and not academic_year:
                row_warnings.append(f'لم يتم العثور على العام الدراسي: {academic_year_name}')

            normalized['academic_year_name'] = academic_year.name if academic_year else academic_year_name
            normalized['row_num'] = index
    ...
    def normalize_row(self, raw_row):
        ...
        normalized['education_level'] = normalized.get('education_level', '').strip()
        normalized['grade_level'] = normalized.get('grade_level', '').strip()
        normalized['academic_year'] = normalized.get('academic_year', '').strip()
    ...
    def build_student(self, row):
        grade = self.find_grade(
            ...
            education_level_name=row.get('education_level', '')
        )
    ...
    def find_education_level(self, education_level_name):
        education_level_name = self.clean_cell(education_level_name)

        if not education_level_name:
            return None

        return (
            EducationLevel.objects.filter(name__iexact=education_level_name).first()
            or EducationLevel.objects.filter(name__icontains=education_level_name).first()
        )

    def find_grade(self, grade_name, education_level_name=''):
        ...
        queryset = GradeLevel.objects.select_related('education_level').all()
        ...
        grade = queryset.filter(name__iexact=grade_name).first()
        if grade:
            return grade

        grade = queryset.filter(name__icontains=grade_name).first()
        if grade:
            return grade

        if not education_level_name:
            exact_matches = GradeLevel.objects.select_related('education_level').filter(
    ...
    def find_academic_year(self, academic_year_name):
        ...
        return (
            AcademicYear.objects.filter(name__iexact=academic_year_name).first()
            or AcademicYear.objects.filter(name__icontains=academic_year_name).first()
        )
