# payments/views.py
# نسخة منظمة ومنظفة - لا تحتوي على التكرارات القديمة أو الدوال المعاد تعريفها
...
def all_payments(request):
    """عرض كل المدفوعات والأقساط مع البحث والفلاتر ودعم الخصومات"""
    user_role = get_user_role(request.user)
    permissions = get_payment_permissions(request.user)
    ...
    if search_query:
        payments_qs = payments_qs.filter(
            ...
            Q(applied_discount__discount_setting__name__icontains=search_query)
        )
    ...
    if discount_filter == 'with_discount':
        payments_qs = payments_qs.filter(discount_amount__gt=0)
    elif discount_filter == 'without_discount':
        payments_qs = payments_qs.filter(
            Q(discount_amount__isnull=True) |
            Q(discount_amount=0)
        )
    elif discount_filter == 'approved_discount':
        payments_qs = payments_qs.filter(
            discount_amount__gt=0,
            applied_discount__isnull=False
        )
    elif discount_filter == 'manual_discount':
        payments_qs = payments_qs.filter(
            discount_amount__gt=0,
            applied_discount__isnull=True
        )
    ...
    summary = payments_qs.exclude(
        ...
        count=Count('id'),
        discounted_count=Count('id', filter=Q(discount_amount__gt=0)),
    )
    ...
    total_before_discount = total_required + total_discounts
    ...
    context = {
        ...
        'selected_discount_filter': discount_filter,
        'date_from': date_from,
        'date_to': date_to,
        ...
        'discounted_count': summary['discounted_count'] or 0,
...
def pay_installment(request, pk):
    ...
    # ============================================================
    # طلبات الخصم المنتظرة لهذا الطالب
    # ============================================================
    try:
        pending_discount_requests = StudentDiscount.objects.filter(
            student=student,
            status='PENDING'
        ).select_related(
            'discount_setting',
        ...
        pending_discount_requests_count = pending_discount_requests.count()

    except Exception as e:
        print(f"تعذر تحميل طلبات الخصم المنتظرة للطالب: {e}")
        pending_discount_requests = []
        pending_discount_requests_count = 0
    # ============================================================
    # إنشاء قسط جديد
    # ============================================================
    ...
    context = {
        ...
        'pending_discount_requests_count': pending_discount_requests_count,
...
def receipt(request, pk):
    """عرض وطباعة إيصال الدفع مع توضيح بيانات الخصم"""
    tuition = get_object_or_404(
        Tuition.objects.select_related(
            'student',
            'student__grade_level',
    ...
    discount_scope = ''
    discount_status = ''
    discount_setting = None

    if getattr(tuition, 'applied_discount', None):
        try:
            discount_setting = tuition.applied_discount.discount_setting
            discount_name = discount_setting.name
            discount_scope = discount_setting.get_application_scope_display()
            discount_status = tuition.applied_discount.get_status_display()
        except Exception:
            discount_name = 'خصم معتمد'
            discount_scope = 'غير محدد'
            discount_status = 'مطبق'
    elif tuition.discount_amount and tuition.discount_amount > 0:
        discount_name = 'خصم يدوي'
        discount_scope = 'على القسط الحالي'
        discount_status = 'مطبق'

    amount_before_discount = (
        (tuition.amount_tuition or Decimal('0.00')) +
        (tuition.discount_amount or Decimal('0.00'))
    )
...
def financial_reports(request):
    """لوحة التقارير المالية العامة مع دعم الخصومات"""
    permissions = get_payment_permissions(request.user)
    user_role = get_user_role(request.user)
    ...
    totals = active_installments.aggregate(
        ...
        discounted_count=Count('id', filter=Q(discount_amount__gt=0)),
        paid_installments_count=Count('id', filter=Q(amount_paid__gt=0)),
    )
    ...
    discounted_installments_count = totals['discounted_count'] or 0
    paid_installments_count = totals['paid_installments_count'] or 0

    # ============================================================
    # خصومات معتمدة وخصومات يدوية
    # ============================================================
    approved_discount_data = active_installments.filter(
        discount_amount__gt=0,
        applied_discount__isnull=False
    ).aggregate(
    ...
    manual_discount_data = active_installments.filter(
        discount_amount__gt=0,
        applied_discount__isnull=True
    ).aggregate(
        total=Sum('discount_amount'),
    ...
    approved_discount_total = approved_discount_data['total'] or Decimal('0.00')
    approved_discount_count = approved_discount_data['count'] or 0

    manual_discount_total = manual_discount_data['total'] or Decimal('0.00')
    manual_discount_count = manual_discount_data['count'] or 0
    ...
    pending_discount_requests = []
    pending_discount_requests_count = 0
    pending_discount_requests_total = Decimal('0.00')

    try:
        from school_settings.models import StudentDiscount

        pending_discount_qs = StudentDiscount.objects.filter(
            status='PENDING'
        ).select_related(
            'student',
            'discount_setting',
        ...
        pending_discount_requests_count = pending_discount_qs.count()
        pending_discount_requests_total = pending_discount_qs.aggregate(
            total=Sum('applied_amount')
        )['total'] or Decimal('0.00')

        pending_discount_requests = pending_discount_qs[:5]

    except Exception as e:
        print(f"تعذر تحميل طلبات الخصومات المنتظرة: {e}")
    ...
    # ============================================================
    # أعلى الخصومات المعتمدة
    # ============================================================
    discount_settings_stats = active_installments.filter(
        discount_amount__gt=0,
        applied_discount__isnull=False
    ).values(
        'applied_discount__discount_setting__name',
        'applied_discount__discount_setting__application_scope',
    ).annotate(
        count=Count('id'),
    ...
    for item in grade_stats:
        ...
        discounts = item['total_discounts'] or Decimal('0.00')
        before_discount = required + discounts
    ...
    context = {
        ...
        'total_discounts': total_discounts,
        'discounted_installments_count': discounted_installments_count,
        'approved_discount_total': approved_discount_total,
        'approved_discount_count': approved_discount_count,
        'manual_discount_total': manual_discount_total,
        'manual_discount_count': manual_discount_count,

        'pending_discount_requests': pending_discount_requests,
        'pending_discount_requests_count': pending_discount_requests_count,
        'pending_discount_requests_total': pending_discount_requests_total,
        ...
        'payment_methods_stats': payment_methods_stats,
        'collectors_stats': collectors_stats,
        'discount_settings_stats': discount_settings_stats,
...
def daily_report(request):
    """التقرير اليومي للمدفوعات مع دعم الخصومات"""
    try:
        ...
        approved_discount_data = daily_payments.filter(
            discount_amount__gt=0,
            applied_discount__isnull=False
        ).aggregate(
            total=Sum('discount_amount'),
        ...
        manual_discount_data = daily_payments.filter(
            discount_amount__gt=0,
            applied_discount__isnull=True
        ).aggregate(
            total=Sum('discount_amount'),
        ...
        discount_stats = {
            'total_discounts': stats_data['total_discounts'] or Decimal('0.00'),
            'discounted_count': stats_data['discounted_count'] or 0,
            'approved_discount_total': approved_discount_data['total'] or Decimal('0.00'),
            'approved_discount_count': approved_discount_data['count'] or 0,
            'manual_discount_total': manual_discount_data['total'] or Decimal('0.00'),
            'manual_discount_count': manual_discount_data['count'] or 0,
        ...
        discount_settings_stats = daily_payments.filter(
            discount_amount__gt=0,
            applied_discount__isnull=False
        ).values(
            'applied_discount__discount_setting__name',
        ...
        # ============================================================
        # إحصائيات الساعات بطريقة آمنة على Python
        # ============================================================
        hourly_map = {}
...
def monthly_report(request):
    """التقرير الشهري للمدفوعات مع توضيح الخصومات"""
    user_role = get_user_role(request.user)
    permissions = get_payment_permissions(request.user)
    ...
    stats_data = payments.aggregate(
        ...
        total_required_after_discount=Sum('amount_tuition'),
        total_discounts=Sum('discount_amount'),
        discounted_installments=Count('id', filter=Q(discount_amount__gt=0)),
    )
    ...
    monthly_stats = {
        ...
        'total_required_after_discount': stats_data['total_required_after_discount'] or Decimal('0.00'),
        'total_discounts': stats_data['total_discounts'] or Decimal('0.00'),
        'discounted_installments': stats_data['discounted_installments'] or 0,
        'active_days': 0,
    }

    monthly_stats['total_before_discount'] = (
        monthly_stats['total_required_after_discount'] +
        monthly_stats['total_discounts']
    )
    ...
    # ============================================================
    # ملخص الخصومات اليدوية
    # ============================================================
    manual_discount_total = payments.filter(
        discount_amount__gt=0,
        applied_discount__isnull=True
    ).aggregate(
        total=Sum('discount_amount'),
    ...
    manual_discount_summary = {
        'amount': manual_discount_total['total'] or Decimal('0.00'),
        'count': manual_discount_total['count'] or 0,
    }
    ...
    context = {
        ...
        'manual_discount_summary': manual_discount_summary,
...
def overdue_payments(request):
    """صفحة متابعة الأقساط المتأخرة وغير المسددة مع دعم الخصومات"""
    permissions = get_payment_permissions(request.user)
    user_role = get_user_role(request.user)
    ...
    if discount_filter == 'with_discount':
        overdue_qs = overdue_qs.filter(discount_amount__gt=0)
    elif discount_filter == 'without_discount':
        overdue_qs = overdue_qs.filter(
            Q(discount_amount__isnull=True) |
            Q(discount_amount=0)
    ...
    if sort_by == 'amount_desc':
        ...
        overdue_qs = overdue_qs.order_by('-discount_amount', 'due_date')
    ...
    for item in overdue_qs:
        ...
        amount_tuition = item.amount_tuition or Decimal('0.00')
        amount_paid = item.amount_paid or Decimal('0.00')
        amount_before_discount = amount_tuition + discount_amount

        total_required += amount_tuition
    ...
    for row in overdue_items:
        ...
        if grade_name not in grade_summary:
            grade_summary[grade_name] = {
                ...
                'amount': Decimal('0.00'),
                'discount_amount': Decimal('0.00'),
                'before_discount': Decimal('0.00'),
            }

        grade_summary[grade_name]['count'] += 1
        grade_summary[grade_name]['amount'] += row['remaining']
        grade_summary[grade_name]['discount_amount'] += row['discount_amount']
        grade_summary[grade_name]['before_discount'] += row['amount_before_discount']
